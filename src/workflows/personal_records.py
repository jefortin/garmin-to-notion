...
def main():
    ...
    for record in filtered_records:
        ...
        existing_pr_record = get_existing_record(notion_client, database_id, activity_name)
        existing_date_record = get_record_by_date_and_name(notion_client, database_id, activity_date, activity_name)

        if existing_date_record:
            update_record(notion_client, existing_date_record['id'], activity_date, value, pace, activity_name, True)
            ...
            try:
                ...
                if date_prop and date_prop.get('date') and date_prop['date'].get('start'):
                    ...
                    if activity_date > existing_date:
                        update_record(notion_client, existing_pr_record['id'], existing_date, None, None, activity_name,
                                      False)
                        print(f"Archived old record: {activity_type} - {activity_name}")

                        write_new_record(notion_client, database_id, activity_date, activity_type, activity_name,
                                         typeId, value, pace)
                    ...
                    update_record(notion_client, existing_pr_record['id'], activity_date, value, pace, activity_name,
                                  True)
                ...
                write_new_record(notion_client, database_id, activity_date, activity_type, activity_name, typeId, value,
                                 pace)
        else:
            write_new_record(notion_client, database_id, activity_date, activity_type, activity_name, typeId, value,
                             pace)
